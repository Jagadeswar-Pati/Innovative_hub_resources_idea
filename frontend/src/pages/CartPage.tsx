...
const PLUS_DEBOUNCE_MS = 420;

const CartPage = () => {
  const { items, totalItems, totalPrice, removeFromCart, updateQuantity } = useCart();
  const lastPlusAtRef = useRef<Record<string, number>>({});

  const handleIncrement = useCallback(
    (productId: string, currentQty: number, stock: number) => {
      const now = Date.now();
      if (now - (lastPlusAtRef.current[productId] || 0) < PLUS_DEBOUNCE_MS) return;
      if (stock > 0 && currentQty >= stock) return;
      lastPlusAtRef.current[productId] = now;
              ...
              <div key={product._id} className="flex flex-col sm:flex-row gap-3 sm:gap-4 bg-card border border-border rounded-xl p-3 sm:p-4 min-h-[132px] sm:min-h-[112px]">
                ...
                <div className="flex sm:flex-col items-center sm:items-end justify-between sm:justify-start gap-2 pt-2 sm:pt-0 border-t sm:border-t-0 border-border sm:min-w-[140px]">
                  <div className="flex items-center border border-border rounded-lg shrink-0">
                    <button type="button" onClick={() => updateQuantity(product._id, quantity - 1)} className="p-3 sm:p-2 hover:bg-secondary active:bg-secondary/80 touch-manipulation min-h-[44px] min-w-[44px] flex items-center justify-center"><Minus className="w-4 h-4" /></button>
                    <span className="px-3 sm:px-3 text-sm min-w-[2.75rem] text-center tabular-nums font-medium">{quantity}</span>
                    <button
                      type="button"
                      disabled={atStock}
                      onClick={() => handleIncrement(product._id, quantity, product.stock)}
