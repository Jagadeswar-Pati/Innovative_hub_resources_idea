...
const EShopHomePage = ({ useMainLayout = false }: EShopHomePageProps) => {
  ...
  const filteredProducts = useMemo(() => {
    ...
    // User-friendly: split into words, product must match ALL words (in any field). Order doesn't matter.
    ...
    return products.filter((p) =>
      words.every(
        (word) =>
          name(p).includes(word) ||
          desc(p).includes(word) ||
          cat(p).includes(word)
      )
    );
  ...
  const content = (
    <>
      <SEO
        title="E-Shop"
        description="Shop electronic components, robotics kits, microcontrollers, and DIY project materials at Innovative Hub. Quality parts at competitive prices."
        path="/eshop"
      />
