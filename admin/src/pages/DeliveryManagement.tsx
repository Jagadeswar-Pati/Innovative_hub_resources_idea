...
const DeliveryManagement: React.FC = () => {
  ...
  const [formState, setFormState] = useState({ state: '', defaultShippingCharge: 0, manualBaseCharge: 0, enabled: true });
  ...
  const openAddState = () => {
    ...
    setFormState({ state: '', defaultShippingCharge: 0, manualBaseCharge: 0, enabled: true });
  ...
  const openEditState = (row: DeliveryStateRow) => {
    ...
    setFormState({
      ...
      manualBaseCharge: row.manualBaseCharge,
      enabled: row.enabled,
    });
    setStateModalOpen(true);
  };
  ...
  const saveState = async () => {
    ...
    try {
      if (editingState) {
        await deliveryApi.updateState(editingState.id, formState);
        toast({ title: 'Updated', description: 'State charge updated' });
      } else {
        await deliveryApi.createState(formState);
    ...
    <div className="space-y-6 animate-fade-in">
      ...
      <Tabs defaultValue="states" className="w-full">
        ...
        <TabsContent value="states" className="space-y-4 mt-4">
          <div className="flex justify-between items-center">
            <p className="text-sm text-muted-foreground">Add state and set default and manual base shipping charges.</p>
            ...
            <table className="w-full">
              <thead>
                <tr className="border-b border-border bg-muted/50">
                  ...
                  <th className="text-left p-3 text-sm font-medium">Default Charge</th>
                  <th className="text-left p-3 text-sm font-medium">Manual Base Charge</th>
              ...
              <tbody>
                {states.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="p-6 text-center text-muted-foreground">No states configured. Add one to set delivery charges.</td>
                  </tr>
                ) : (
                  states.map((row) => (
                    <tr key={row.id} className="border-b border-border">
                      ...
                      <td className="p-3">{formatCurrency(row.manualBaseCharge)}</td>
        ...
        <TabsContent value="dashboard" className="mt-4">
          {dashboard && (
            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-4">
              ...
              <div className="stat-card">
                <p className="text-sm text-muted-foreground">By Default Method</p>
                <p className="text-2xl font-bold text-foreground">{dashboard.ordersByDeliveryMethod?.default ?? 0}</p>
              </div>
              <div className="stat-card">
                <p className="text-sm text-muted-foreground">By Manual Method</p>
      ...
      >
        <div className="space-y-4">
          ...
          <div>
            <Label>Manual Method Base Charge (₹)</Label>
            <Input
              type="number"
              min={0}
              step={1}
              value={formState.manualBaseCharge || ''}
              onChange={(e) => setFormState({ ...formState, manualBaseCharge: Number(e.target.value) || 0 })}
            />
          </div>
